...
logModel = glm(sepsis~uac + ecmo + lethargy + fio2_threshold + sbp + map + gest_age + weight + hr_delta + rr_delta + cvl + hr + age + temp, family=binomial, data = train)
...
log.fit = ifelse(log.fit > 0.2, '1', '0')

library(caret)
print('Test confusion matrix')
confusionTabl2 = confusionMatrix(as.factor(log.fit), as.factor(test$sepsis), positive = '1')
confusionTabl2
```
...
```{r}
misclassification = sum(log.fit != test$sepsis)/length(test$sepsis)
print(paste('Accuracy', 1-misclassification))
```

```{r}
log.fit = predict(logModel, newdata = test, type='response')
pred <- prediction(log.fit, test$sepsis)
perf <- ROCR::performance(pred, 'tpr', 'fpr')
plot(perf, main= "Mean Imputed Dataframe")
abline(a=0, b=1, lty = 2, col = 3)

aucROCR <- ROCR::performance(pred, measure = "auc")
aucROCR <- aucROCR@y.values[[1]]
legend('bottomright',legend = c(round(aucROCR,3), "Random Guessing"),col = c(1,3), lty = c(1, 2), title = 'ROC-AUC')
...
```{r}
spliti = sample.split(df_missingCats$sepsis, SplitRatio=.8)

traini <- subset(df_missingCats, split==TRUE) 
testi <- subset(df_missingCats, split ==FALSE)
```

```{r}

logModel2 = glm(sepsis~uac + ecmo + lethargy + fio2_threshold + sbp + map + gest_age + weight + hr_delta + rr_delta + cvl + age + hr, family=binomial, data = traini)
...
```{r}
log.fit2 = predict(logModel2, newdata = testi, type = 'response')
```

```{r}
log.fit2 = predict(logModel, newdata = testi, type = 'response')
log.fit2 = ifelse(log.fit2 > 0.2, '1', '0')

library(caret)
print('Test confusion matrix')
confusionTabl22 = confusionMatrix(as.factor(log.fit2), as.factor(testi$sepsis), positive = '1')
confusionTabl22
```
...
```{r}
misclassification2 = sum(log.fit2 != testi$sepsis)/length(testi$sepsis)
print(paste('Accuracy', 1-misclassification2))
```

```{r}
log.fit2 = predict(logModel, newdata = testi, type='response')
pred2 <- prediction(log.fit2, testi$sepsis)
perf2 <- ROCR::performance(pred2, 'tpr', 'fpr')
plot(perf, main= "ROC Curve for Logistic Regression")
abline(a=0, b=1, lty = 2, col = 3)

aucROCR2 <- ROCR::performance(pred2, measure = "auc")
aucROCR2 <- aucROCR2@y.values[[1]]
aucROCR2
legend('bottomright',legend = c(round(aucROCR2,3), "Random Guessing"),col = c(1,3), lty = c(1, 2), title = 'ROC-AUC')
```
```{r}
summary(logModel)
```
