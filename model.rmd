...
logModel = glm(sepsis~. +polym(uac, ecmo, lethargy, fio2_threshold, sbp, map, gest_age, weight, hr_delta, rr_delta), family=binomial, data = train)
...
```{r}
log.fit = ifelse(log.fit > 0.22, '1', '0')
```
...
logModel2 = glm(sepsis~. +polym(uac, ecmo, lethargy, fio2_threshold, sbp, map, gest_age, weight, hr_delta, rr_delta), family=binomial, data = traini)
...
```{r}
log.fit2 = ifelse(log.fit2 > 0.22, '1', '0')
```
